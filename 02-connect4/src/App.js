...
class Game extends React.Component {
  ...
  //métodos que ayudan a determinar si hay 4 piezas en una fila:
...
checkVictory(row,col){
  ...
  //permitirá saber en qué fila debe colocarse una pieza cuando se descarta en una columna específica.
  findAvailableRow(col){
    ...
    return -1;
      ...
      handleClick(row, col){
        ...
        //detener el juego una vez que alguien haya ganado.
    ...
    //Agregar botón de reinicio de juego
    restart(){
      var cells = [];
      for(let i = 0; i < 6; i++ ){
        cells.push(new Array(7).fill(0));
  ...
  render() {
      return (
          <div>
    <h1>{this.state.winner > 0 ?  this.state.winner == 1? "Black Wins":"Red Wins": this.state.player? "Blacks Turn" : "Reds Turn"} </h1>          <Board cells = {this.state.cells} handleClick = {this.handleClick}/>
